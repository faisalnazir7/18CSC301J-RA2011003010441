...
const Navbar = (props: Props) => {
    const {palette} = useTheme(); // useTheme grabs the Themes from theme.ts
    ...
    <FlexBetween gap="2rem">
        <Box sx={{"&hover":{color: palette.primary[100]}}}>
            <Link to="/" onClick={()=> setSelected("dashboard")} style={{color: selected === "dashboard" ? "inherit" : palette.grey[700], textDecoration: "inherit"}}>
                {/* Upon clicking the selected is stored as dashboared and upon clickin inherits grey 700 colour */}
                dashboard
            </Link>
        </Box>
        <Box sx={{"&hover":{color: palette.primary[100]}}}>
